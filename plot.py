...
def plot_explained_var_ratio(ratio, saved=False):
    ...
    plt.xlabel('PCA')
    if saved:
        plt.savefig('outputs/pca_explained_var_ratio.png', dpi=800)
    plt.show()
    pass
...
def plot_pca_analysis(feat, target, saved=False):
    ...
    if saved:
        plt.savefig('outputs/pca_samples_2&3d.png', dpi=800)
    plt.show()
    pass

def plot_kpca_analysis(feat, target, ker, saved=False):
    plt.figure(figsize=(10, 5))
    ax1 = plt.subplot(121)
    ...
    ax1.set_xlabel('PC1')
    ax1.set_ylabel('PC2')
    ax1.set_title('Samples in 2D space when kernel of KPCA is ' + ker)
    ax2 = plt.subplot(122, projection='3d')
    ...
    ax2.set_ylabel('PC2')
    ax2.set_zlabel('PC3')
    ax2.set_title('Samples in 3D space when kernel of KPCA is ' + ker)
    if saved:
        plt.savefig('outputs/kpca_' + ker + '_samples_2&3d.png', dpi=500)
    plt.show()
    pass
...
def plot_lle_analysis(feat, target, n_neighbor, saved=False):
    ...
    if saved:
        plt.savefig('outputs/lle_samples_' + str(n_neighbor) + '_2&3d.png', dpi=800)
    plt.show()
    pass

def plot_lle23_analysis(feat2d, feat3d, target, n_neighbor, saved=False):
    plt.figure(figsize=(10, 5))
    ax1 = plt.subplot(121)
